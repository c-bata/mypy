...
class MypycMagics(Magics):
    ...
    def mypyc(self, line, cell):
        ...
        module_name = "_mypyc_magic_" + hashlib.sha1(str(key).encode('utf-8')).hexdigest()
        module_path = os.path.join(lib_dir, module_name + self.so_ext)
        need_mypycify = not os.path.isfile(module_path)

        extension = None
        if need_mypycify:
            extensions = self._mypycify(module_name, code, lib_dir, args, quiet=args.quiet)
    ...
    def _get_build_extension(self, extension=None, lib_dir=None, temp_dir=None, _build_ext=build_ext):
